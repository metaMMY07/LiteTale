...
class NovelDownloadInfoPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => NovelDownloadInfoCubit(novelId)..load(),
      child: Scaffold(
        appBar: AppBar(
          title: const Text('下载详情'),
          actions: [
            BlocBuilder<NovelDownloadInfoCubit, NovelDownloadInfoState>(
        ...
        body: BlocBuilder<NovelDownloadInfoCubit, NovelDownloadInfoState>(
          builder: (context, state) {
              ...
              return Center(
                child: Column(
                  ...
                  children: [
                    ...
                    ElevatedButton(
                      onPressed: () {
                        context.read<NovelDownloadInfoCubit>().load();
                      },
                      child: const Text('重试'),
                    ),
                  ],
            ...
            if (state is NovelDownloadInfoLoaded) {
              return _NovelDownloadInfoContent(
                download: state.download.novelDownload,
...
class _NovelDownloadInfoContent extends StatelessWidget {
  ...
  Future<void> _navigateToReader(
    ...
    final currentState = context.read<NovelDownloadInfoCubit>().state;
    if (currentState is! NovelDownloadInfoLoaded) return;
    ...
    // 返回后更新历史记录
    context.read<NovelDownloadInfoCubit>().loadHistory();
  }
